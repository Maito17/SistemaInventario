...
class RespaldoDatabase:
    ...
    def respaldar_base_datos(self):
        """Crear respaldo SQL de la BD con pg_dump."""
    ...
    def respaldar_base_datos_json(self):
        ...
        try:
            ...
            # Comprimir
            with gzip.open(respaldo_gz, 'wt', encoding='utf-8') as f_out:
            ...
            print(f"✅ Respaldo JSON creado: {respaldo_gz.name} ({self.ultimo_tamaño:.2f} MB)")
            print(f"🔐 SHA256: {self.ultimo_checksum[:16]}...")
            print(f"🌐 Este formato es compatible con PostgreSQL, MySQL, SQLite y otros")
            return True
