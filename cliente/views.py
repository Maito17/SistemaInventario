...
def clientes_lista(request):
    ...
    credito_filtro = request.GET.get('credito', '').strip()
    ...
    if query:
        clientes = clientes.filter(
            ...
            Q(ruc_cedula__icontains=query) |
            Q(email__icontains=query) |
            Q(telefono__icontains=query)
        )
    
    if credito_filtro == 'activo':
        clientes = clientes.filter(credito_activo=True)
    elif credito_filtro == 'inactivo':
        clientes = clientes.filter(credito_activo=False)
    
    # Métricas (sobre todos los clientes, sin filtro)
    todos_clientes = Cliente.objects.filter(user=request.user)
    total_clientes = todos_clientes.count()
    clientes_con_credito = todos_clientes.filter(credito_activo=True).count()
    
    context = {
        'clientes': clientes,
        'query': query,
        'credito_filtro': credito_filtro,
