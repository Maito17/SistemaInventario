...
# Cargar la clave de Gemini desde el entorno y configurar
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for field, error_list in form.errors.items():
                label = form.fields[field].label or field if field in form.fields else field
                for err in error_list:
                    errores.append(f"{label}: {err}")
            detalle = ' | '.join(errores) if errores else 'Verifica los datos.'
            messages.error(request, f"❌ Error al guardar: {detalle}")
    else:
        if config:
