...
class PasswordResetToken(models.Model):
    """Token seguro para recuperación de contraseña (almacenado en BD)."""
    ...
    def __str__(self):
        return f"Reset token para {self.user.username} ({'usado' if self.used else 'activo'})"

    def is_valid(self):
        return not self.used and self.expires > timezone.now()

    @classmethod
    def cleanup_expired(cls):
        """Elimina tokens expirados o usados."""
        cls.objects.filter(
            models.Q(expires__lt=timezone.now()) | models.Q(used=True)
        ).delete()
